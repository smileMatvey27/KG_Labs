...
bool initialize(GLFWwindow* const window) {
	...
	vk_swapchain_resize_width = uint32_t(width);
	vk_swapchain_resize_height = uint32_t(height);
...
FrameData prepare() {
	...
	if (vk_swapchain_resize_require) {
		std::cerr << "[prepare] resize_require = true, rebuilding\n";
		vk_swapchain_resize_require = false;
		rebuildSwapchain(vk_swapchain_resize_width, vk_swapchain_resize_height);
	}

retry_acquire:
	VkResult acquire_result = vkAcquireNextImageKHR(context.device, vk_swapchain, UINT64_MAX,
	...
	case VK_SUBOPTIMAL_KHR: {
		std::cerr << "[prepare] acquire result = " << acquire_result
	...
	default:
		std::cerr << "[prepare] acquire FAILED, result = " << acquire_result << "\n";
		return {};
	}
...
void submitAndPresent() {
	...
	if (result == VK_ERROR_OUT_OF_DATE_KHR) {
		std::cerr << "[present] OUT_OF_DATE, rebuilding\n";
		rebuildSwapchain(vk_swapchain_resize_width, vk_swapchain_resize_height);
	}
	else if (result == VK_SUBOPTIMAL_KHR) {
	}
	else if (result != VK_SUCCESS) {
		std::cerr << "[present] present FAILED, result = " << result << "\n";
	}
}
